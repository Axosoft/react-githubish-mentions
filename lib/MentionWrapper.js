...
var MentionWrapper = function (_Component) {
  ...
  function MentionWrapper(props) {
    ...
    _this.handleKeyDown = function (e) {
      var _this$state = _this.state,
          ...
          activeTrigger = _this$state.activeTrigger;
    ...
    _this.selectItem = function (active) {
      return function (e) {
        var _this$state2 = _this.state,
            activeTrigger = _this$state2.activeTrigger,
            options = _this$state2.options;

        var preMention = _this.ref.value.substr(0, _this.ref.value.lastIndexOf(activeTrigger));
        var option = options[active];
        var mention = activeTrigger + option.value;
  ...
  _createClass(MentionWrapper, [{
    ...
    value: function maybeMention() {
      ...
      var activeTrigger = undefined;
      this.triggers.forEach(function (trigger) {
        if (lastToken.includes(trigger) && lastToken.indexOf(trigger) === 0) {
          activeTrigger = trigger;
        }
      });
      var textBeforeTrigger = lastToken.split(activeTrigger)[0];
      var textAfterTrigger = lastToken.split(activeTrigger)[1];

      // check if the text befor the caret ends with the last word
      var triggerIdx = textBeforeCaret.endsWith(lastToken) ? textBeforeTrigger.length : -1;

      var keystrokeTriggered = this.triggers.indexOf(activeTrigger);
    ...
    value: function render() {
      var _props = this.props,
          ...
          componentInputType = _props.componentInputType,
          CustomComponent = _props.CustomComponent,
          getRef = _props.getRef,
          containerStyle = _props.containerStyle,
          textWrapperClassName = _props.textWrapperClassName,
          inputProps = _objectWithoutProperties(_props, ["children", "component", "componentInputType", "CustomComponent", "getRef", "containerStyle", "textWrapperClassName"]);
      ...
      var componentProps = _extends({}, inputProps, {
        ref: this.inputRef,
      ...
      if (component) {
        inputElement = _react2.default.createElement(component, componentProps);
      } else {
        inputElement = _react2.default.createElement("textarea", componentProps);
      }
