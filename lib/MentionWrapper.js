...
var MentionWrapper = function (_Component) {
  ...
  function MentionWrapper(props) {
    ...
    _this.selectItem = function (active) {
      return function (e) {
        ...
        var preMention = _this.ref.value.substr(0, triggerIdx - 1);
        var option = options[active];
        var mention = _this.replace(option, _this.ref.value[triggerIdx - 1]);
  ...
  _createClass(MentionWrapper, [{
    ...
    value: function maybeMention() {
      ...
      // Get active trigger
      var activeTrigger = this.triggers.find(function (trigger) {
        return lastToken.startsWith(trigger);
      }) || "";

      // check if the text before the caret ends with the last word
      var triggerIdx = textBeforeCaret.endsWith(lastToken) ? textBeforeCaret.length - (lastToken.length - activeTrigger.length) : -1;

      // and if that last word starts with a trigger
      var maybeTrigger = textBeforeCaret.substring(triggerIdx - activeTrigger.length, triggerIdx);
      ...
      if (keystrokeTriggered !== -1) {
        (function () {
          ...
          var query = textBeforeCaret.substring(triggerIdx);
    ...
    value: function render() {
      var _props = this.props,
          ...
          CustomInputComponent = _props.CustomInputComponent,
          CustomComponent = _props.CustomComponent,
          getRef = _props.getRef,
          containerStyle = _props.containerStyle,
          textWrapperClassName = _props.textWrapperClassName,
          inputProps = _objectWithoutProperties(_props, ["children", "CustomInputComponent", "CustomComponent", "getRef", "containerStyle", "textWrapperClassName"]);
      ...
      if (CustomInputComponent) {
        inputElement = _react2.default.createElement(CustomInputComponent, componentProps);
      } else {
